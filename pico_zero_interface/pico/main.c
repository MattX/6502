 ...
 *
 * Receives WRITE payloads via the RX queue and verifies patterns.
...
static uint8_t rx_buf[SPI_SLAVE_MAX_PAYLOAD];

static void verify_data(const uint8_t *data, uint16_t len) {
    rx_count++;
    rx_bytes += len;
...
int main(void) {
    ...
    while (1) {
        ...
        // Drain and verify any received WRITE payloads
        uint n = spi_slave_rx_drain(rx_buf, sizeof(rx_buf));
        if (n > 0) {
            verify_data(rx_buf, n);
        }

        uint32_t now = to_ms_since_boot(get_absolute_time());
        if (now - last_stats >= 2000) {
            spi_slave_stats_t s = spi_slave_get_stats();
            printf("[%lus] wr=%lu (%lu KB) rd=%lu req=%lu rx_err=%lu oflow=%lu proto_err=%lu\n",
