...
// Called when a complete WRITE payload has been received (CS went high).
// |data| points into the RX ring buffer and is only valid during the callback.
...
// Register callback for received WRITE payloads.
...
// Returns current free space in RX ring buffer, in 64-byte units (BUF field).
